...
import { deleteBusinessProfile, updateBusinessProfile } from '@/lib/db';
...
export async function PATCH(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  for (const k of ['name', 'fromName', 'fromTradingAs', 'fromAbn', 'fromAddress', 'fromEmail', 'fromPhone'] as const) {
  ...
  const profile = await updateBusinessProfile(id, updates);
...
export async function DELETE(_req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const { id } = await params;
  const ok = await deleteBusinessProfile(id);
